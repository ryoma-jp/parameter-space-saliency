...
def transform_raw_image(image_path, preprocess=None):
    """Load and preprocess a raw image file.

    Args:
        image_path: Path to the image.
        preprocess: A ``transforms.Compose`` (or any callable) to apply.
                    When ``None``, falls back to the ImageNet default
                    (256 resize → 224 centre-crop → normalise).
